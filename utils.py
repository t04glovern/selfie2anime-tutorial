...
def video_save_images(images, size):
    return video_imsave(video_inverse_transform(images), size)

def video_inverse_transform(images):
    return ((images+1.) / 2) * 255.0

def video_imsave(images, size):
    images = merge(images, size)
    images = cv2.cvtColor(images.astype('uint8'), cv2.COLOR_RGB2BGR)
